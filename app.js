...
async function saveForm(form){
 ...
 if(type==='medication'){
   ...
   const updateHistoricalName=fd.has('updateHistoricalName');
   delete obj.updateHistoricalName;
   obj.active=obj.status==='Active';obj.schedule=obj.frequency||'';
   if(existing&&updateHistoricalName&&String(existing.name||'')!==String(obj.name||'')){
     state.doses.forEach(d=>{if(d.medicationId===obj.id)d.medicationNameSnapshot=obj.name||'Medication'});
   }
